    ...
    async def process_messages( self, websocket ):
        ...
        try:
            ...
            async for message in websocket:
                ...
                ######################################################################
                if 'metadata' not in msg or 'payload' not in msg:
                    await self.__websocket.close( )
                    err = RuntimeError(f'malformed message: {msg}')
                ...
                if 'frontend_id' not in metadata:
                    await self.__websocket.close( )
                    err = RuntimeError(f'frontend_id not in: {metadata}')
                    if self.__abort is not None:
                        self.__abort( err )
                    else:
                        raise err
                ...
                with self.__lock:
                    ...
                    if metadata['direction'] == 'p2j':
                        cb = self.__get_pending(metadata['recipient'])
                        outgo = self.__dequeue_send(metadata['recipient'])
                        if outgo is not None:
                            await websocket.send(serialize(outgo['msg']))
                            await self.__put_pending(metadata['recipient'],outgo['cb'])
                        if cb is not None:
                            if inspect.isawaitable(cb):
                                await cb(payload['message'])
                            else:
                                cb(payload['message'])
                    elif payload['id'] != 'initialize':
                        ###
                        ### no cb/reply for initialization of session
                        ###
                        if payload['id'] not in self.__incoming_callbacks:
                            raise RuntimeError(f'incoming js request with no callback: {payload}')
                        result = self.__incoming_callbacks[metadata['recipient']](payload['message'])
                        if inspect.isawaitable(result):
                            try:
                                return_message = "an exception occurred in creating the respone"
                                return_message = await result
                                ### FIXME>>> python to typescript message reply format
                                await self.__websocket.send( serialize( { 'id': payload['id'],
                                ...
                                print('************************************************************************************************************************')
                                ### FIXME>>> python to typescript message error format
                                await self.__websocket.send( serialize( { 'id': payload['id'],
