                        ...
                        with _COLAB_JS_EVAL_LOCK:
                            ...
                            try:
                                ...
                                _final_js = (
                                    ...
                                    f"console.log('CUBEVIS finalizer: tok={_tok}' , 'arr=', arr, 'len=', arr ? arr.length : 'MISSING');"
                                    f"if(!arr || arr.length===0){{ console.error('CUBEVIS: accumulator missing or empty for {_tok}'); return; }}"
                                    f"const s=arr.join('');"
                                    f"delete window[{_tok_key}];"
                                    f"const msg=JSON.parse(s);"
                                    f"const cb=window[{_pj.dumps(_cb)}];"
                                    f"console.log('CUBEVIS finalizer: cb=', typeof cb);"
                                    f"if(typeof cb==='function'){{cb(msg);}}"
                                    f"try{{const bc=new BroadcastChannel({_pj.dumps(_bc)});"
                                    f"bc.postMessage(msg);bc.close();}}catch(e){{console.error('CUBEVIS bc error',e);}}"
