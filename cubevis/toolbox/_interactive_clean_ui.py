...
class InteractiveCleanUI:
    ...
    def __stop( self, _=None ):
        if not self.__result_future.done():
            logger.debug("InteractiveCleanUI.__stop: setting clean result")
            self.__result_future.set_result(self.__retrieve_result( ))
        else:
            logger.debug("InteractiveCleanUI.__stop: result already set")
        pass
    ...
    def __init__( self, gclean, user_args ):
        ...
        def error_handler( *args, **kwargs ):
            logger.debug( f"InteractiveCleanUI...error_handler: {args}, {kwargs}" )
        ...
        def connection_closed_handler( reason, description ):
            ###
            ### A connection ended but the session is still alive -- the laptop
            ### slept, the network blipped, or the tab was reloaded. Do NOT call
            ### self.__stop( ) here: it resolves __result_future, which exits
            ### the `async with websockets.serve( ... )` block in _task_server( )
            ### and closes the listening socket the frontend is about to
            ### reconnect to.
            ###
            logger.debug( f"InteractiveCleanUI...connection_closed_handler: "
                          f"connection lost ({description}); awaiting reconnection" )

        def reconnect_handler( generation ):
            logger.debug( f"InteractiveCleanUI...reconnect_handler: "
                          f"frontend reconnected (generation={generation})" )
        ...
        ## comm manager for opening communication channels
        self._comm_mgr = self._app_context.comm_mgr

        ## Connection lifecycle: these fire for transient disconnects and are
        ## distinct from on_shutdown, which ends the session for good.
        self._comm_mgr.set_connection_closed_callback( connection_closed_handler )
        self._comm_mgr.set_reconnect_callback( reconnect_handler )

        ## Requests that were in flight when a connection dropped are replayed
        ## once the frontend returns. All interactive clean p2j traffic is
        ## idempotent GUI state, so redelivery is safe. Set to False if that
...
#          os.makedirs(webpage_dirname)
        ...
        for idx, (imid, imdetails) in enumerate(self._clean_targets.items( )):
            ...
            ###
            imdetails['gui']['cube'] = CubeMask( imdetails['path']['residual'], mask=imdetails['path']['mask'],
    ...
    def _build_bokeh( self ):
        ...
        ###
        async def clean_handler( msg, context, self=self ):
            if msg['action'] == 'next' or msg['action'] == 'finish':
                ...
                logger.debug(f"InteractiveCleanUI...clean_handler received shutdown message")
                context.request_shutdown( "user requested stop" )
        ...
        for imid, imdetails in self._clean_targets.items( ):
            ...
            ###
            def convergence_handler( msg, self=self, imid=imid, context=None ):
                                                      ...
                                                      '''function invalid_niter( s ) {
                                                         ...
                                                         if ( state.mode === 'interactive' && cb_obj.origin.name === 'stop' ) {
                                                             if ( confirm( "Are you sure you want to end this interactive clean session and close the GUI?" ) ) {
                                                                 disable( true )
                                                                 console.log('interactive clean: before shutdown')
                                                                 appstate.cube_done(
                                                                     Object.entries(images_state).reduce(
                                                                         (acc,[k,v]) => ({ ...acc, [k]: v.src.masks( ) }),
                                                                         { }
                                                                     ),
                                                                     (msg) => {
                                                                         console.log('interactive clean: after cube done')
                                                                         ctrl_pipe.send(
                                                                             ids[cb_obj.origin.name],
                                                                             { action: 'stop',
                                                                               value: { } },
                                                                             ( msg ) => {
                                                                                     console.log('interactive clean: after shutdown')
        ...
        self._pipe['control'].add_init_script( args=dict( ctrl_pipe=self._pipe['control'],
                                                          ids=self._clean_ids,
                                                          logbutton=self._log_button,
                                                          log=initial_log,
                                               ...
                                               description="interactive clean control pipe" )
...
###     #    output_file(self._imagename+'_webpage/index.html')
    ...
    async def _task_server( self ):
        ...
        if self._comm_mgr.address:
            ...
            ### and it returns every time a socket drops. Keeping the listener
            ### open across those returns is what makes reconnection possible.
            ###
            ### The keepalive parameters make the server notice a dead peer in
            ### ~30s instead of waiting out TCP timeouts. The frontend runs its
            ### own application-level heartbeat, because browsers cannot send
            ### WebSocket ping frames from JavaScript.
            ###
                     ...
                     'update-converge': '''function updateAxisRange(source, range_obj, axis_obj, axis_name) {
                                           ...
                                           function update_convergence( recurse=false ) {
                                               ...
                                               if ( hasprop(appstate,'convergence_data') ) {
                                                   ...
                                                   if ( ! recurse ) {
                                                       /** fetch convergence state because it is missing from last clean exeuction **/
