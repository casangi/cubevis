...
**Correction to this whole subsection, 2026-09 (Chunk 2b/2c, applied
directly to the real code, not left as a plan):** what actually shipped
does not stream a lazy Dask-backed dataset to a client-side
`Canvas.points()` call as this subsection originally proposed. Instead,
`MSv2Backend.query_columns`/`MSv4Backend.query_columns` bin and shade
**server-side** (wherever the backend object actually lives — the
worker process, for a remote session) and return a bounded
`ScatterRenderResult`: a rendered RGBA image plus a handful of small
per-layer arrays (coarse identity-grid data for hover/click-to-exact —
see the Chunk 2c section below — not raw rows). The memory-safety
argument below (a broad selection risking materializing an unbounded
row set) is exactly why this shape was chosen; the mechanism is
different from what this subsection anticipated, but it solves the same
problem this subsection identified, by binning before crossing the
`call_method` boundary rather than after. The "no `max_cells`-style cap
is needed for scatter" reasoning below is subsumed by this — the image
output is bounded by canvas resolution automatically, same conclusion,
different route to it. `RemoteReductionContext.query_columns()` is
consequently a straight relay of this bounded, already-rendered result
(confirmed a plain `call_method` with no special-casing needed — see its
own source comment on why no `_wire_types` registration was required),
not the "Canvas.points() over the wire" design below.
...
- Which execution-context configuration `visplot`'s worker registration
  ...
  immediately, not lazily) — see the Chunk 2d section's "Construction
  blocks" discussion for the latency consequence of that choice.
...
**What had never been exercised before this chunk, at all:** every piece
of the `cubevis.remote` machinery described in Chunks 1/1b/1c/2a/2b —
kernel bootstrap, the `CommMgr` mirror link, generalized eval/exec, the
object registry, and `RemoteReductionContext` specifically — had zero
runtime verification. It existed, and (per Chunks 1/1b/1c's own status
above) had been tested at the framework level, but `RemoteReductionContext`
itself, `visplot`'s specific application of that framework, had never
actually been run.
