...
```bash
PROTOBUF_VER=$(cd ~/develop/casa/casa6-dev && pixi list 2>/dev/null | awk '$1=="libprotobuf"{print $2; exit}') && mamba create -n ms4-py312 python=3.12 ipython websockets anywidget bokeh=3.9 scipy regions certifi xarray zarr 'pyarrow=23' "libprotobuf=$PROTOBUF_VER" dask datashader pytest nodejs
```
