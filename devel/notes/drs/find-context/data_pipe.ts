...
export class DataPipe extends DataSource {
            ...
            if (existing) {
                ...
                if (existingData.sessionId !== this.frontend_id && 
                    ...
                    if (this.conflict_check) {
                        ...
                        console.log('Current session ID:', this.frontend_id);
        ...
        try {
            if (typeof(Storage) !== "undefined") {
                ...
                if (current) {
                    ...
                    if (currentData.sessionId === this.frontend_id) {
    ...
    private handleSessionConflictMessage(message: any): void {
        ...
        const event = new CustomEvent('cubevis_session_conflict', {
            detail: { message, sessionId: this.frontend_id }
        })
    ...
    initialize(): void {
        ...
        var connect_to_server = ( ) => {
            ...
            this.websocket.onmessage = (event: any) => {
                if (typeof event.data === 'string' || event.data instanceof String) {
                    ...
                    if ( 'id' in data && 'direction' in data && 'message' in data ) {
                        ...
                        if (direction === 'error' && (id === 'session_conflict' || id === this.frontend_id)) {
                        ...
                        if ( direction == 'j2p' ) {
                            ...
                            if ( id in this.incoming_callbacks ) {
                                ...
                                this._ws_send( id, { id, direction, message: result, session: this.frontend_id } )
                                //this.websocket.send( serialize({ id, direction, message: result, session: this.frontend_id }))
                            }
                        }
                    } else {
            ...
            this.websocket.onopen = ( ) => {
                if ( ! reconnections ) {
                    this._ws_send( { id: 'initialize',
                                     direction: 'j2p',
                                     session: this.frontend_id,
    ...
    //private _ws_send( recipient: string, message: {[key: string]: any} ): void;
    //private _ws_send( message: {[key: string]: any} ): void;
    ...
    private _ws_send(arg1: any, arg2?: any): void {
        ...
        if ( typeof arg1 === 'string' && arg1.trim().length > 0 ) {
            ...
            recipient = arg1;
            message = arg2!; // '!' tells TS arg2 is guaranteed here by the signature
