...
Chunk 1c implemented and tested; Chunk 2 (`visplot`) designed **and now
implemented and tested**, including against a real cluster kernel (see
the implementation doc's Chunk 2 section for what shipped and how it was
verified); Chunk 3 designed at the architecture level, not yet
implemented, and committed to building on Chunk 1c from the start rather
than being retrofitted onto it later.
...
- **Chunk 2 (`visplot` remote data path) — 2a raster, 2b scatter.** Gives
  ...
  appetite for change). **Status: implemented and tested** — raster and
  scatter both shipped (2a/2b), the hover-probe/click-to-exact redesign
  and `InfoTool` on top of them (2c), and the remote path exercised
  end-to-end first against a local kernel and then against a real
  `zuul06` cluster kernel, full suite passing clean (2d). See the
  implementation doc's Chunk 2 section — in particular, the actual
  scatter aggregation shape that shipped differs from what was designed
  here; that section has the correction.
...
- **Chunk 3 (`iclean`/`gclean` remote data path).** Gives `iclean` a
  ...
  direction; revisit once Chunk 2's pattern is proven out — Chunk 2 is
  now fully implemented and tested, including against a real cluster
  kernel, so this is no longer blocked on anything but priority. **Status:
...
4. **Chunk 2a** (raster) — done.
5. **Chunk 2b** (scatter) — done, on a different aggregation shape than
   originally designed here — see the implementation doc's Chunk 2
   correction.
6. **Chunk 2d** (remote-path validation, real and local kernel) — done;
   the implementation doc has the full account.
7. **Chunk 3** (`iclean`/`gclean`) — not blocked on anything since Chunk
   2's pattern is now proven end to end, including against a real
   cluster kernel; revisit per priority direction.
