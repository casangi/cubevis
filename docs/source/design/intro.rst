...
This design document describes the history, motivation and design of the cubevis
system. This system includes the cubevis Python package and the cubevis application
framework. The cubevis Python package is used by pipeline developers and other Python
users to access cubevis functionality from Python. The cubevis application framework
also uses the cubevis Python package. The design of the cubevis system allows the
...
to function. All of the cubevis functionality will **not** be available in all of these
...
receiving a response). We navigate these limitations as the cubevis system is built out
...
not be accessible. One important area that the cubevis system must support is pipeline
...
In the headless setting, the only process involved from the cubevis perspective is the
Python process that imported the cubevis Python package, and the only display option
...
the cubevis package into their python environment using something like pip.  Because we
want to avoid OS specific binary applications, all cubevis functionality in this setting is
...
the cubevis Python package is isolated from the incompatibilities which such implementations
entail.

In the terminal setting, the user’s web browser is the best display option. It is (hopefully)
compatible with the cubevis application framework, and the user already has a browser that
...
The cubevis system takes a different approach. The cubevis Python package and its application
framework, Electron, are leveraged to create a platform for hosting all of the visualization
applications in the cubevis system. All of these applications will be hosted by a single, running
...
The **same** cubevis python package used in the other settings is imported into the IPython
...
they use the same cubevis Python implementation.
...
same underlying protocol, many of the plots generated by the common cubevis Python package will
...
Most of the individual plots that the cubevis Python package is capable of creating should
...
The cubevis design documents are available for more detailed discussions about the implementation
