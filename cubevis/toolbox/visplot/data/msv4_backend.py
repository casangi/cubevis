...
class MSv4Backend(XArrayReader):
    ...
    def axis_info(self, axis, selection=None, query="columns"):
        ...
        # rather than assumed.  This branch hardcoded "spw {id}", so it
        # would label a spectral window *name* as
        # "spw ALMA_RB_07#BB_2#SW-01#FULL_RES" -- the same mislabelling
        # §8.11 fixed in MSv2 and never applied here, because MSv4 was
        # assumed always to carry a numeric id.  It does not: xarray-ms
        # 0.5.6 provides only the name.
        #
        # A name is self-describing and takes no prefix; a numeric id
        # needs one, because "137" alone says nothing about what it
        # indexes.
        ...
        if spw_id is None:
            ...
            # say nothing rather than pick one and be wrong about the rest.
            context = None
    ...
    @classmethod
    def _partition_spw_id(cls, ds):
        """Bare identity for grouping and filtering; kind discarded.

        Callers that *display* the value must use
        ``_partition_spw_ident`` and branch on the kind -- a DATA_DESC_ID
        printed as "spw 3" names something ``spw='3'`` does not select.
        """
    ...
    def probe_scatter_region(
        ...
        Mirrors ``MSv2Backend.probe_scatter_region`` exactly, except this
        backend's ``_lazy_quantity`` takes no ``ds`` argument (MSv4 has no
        Axis.U/Axis.V-as-y-axis path -- see ``_lazy_quantity`` above).
        See ``XArrayReader.probe_scatter_region`` for the full contract.

        Like ``probe_raster_pixel``/``identity_tables`` above, this does
        not special-case single-dish mode (``baseline_id`` is assumed) --
        a partition without that coordinate simply contributes no
        ``bl_range``/``bl_ids``, matching the pre-existing gap in those
        two methods rather than introducing a new one.
        """
        self._require_open()
        if not yaxes:
        ...
        for raw_ds in self._iter_visibility_partitions(selection):
            ...
            for (yaxis, pol) in local_keys:
                ...
                if "baseline_id" in mask_c.dims and "baseline_id" in ds.coords:
